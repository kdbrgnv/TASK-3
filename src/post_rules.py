...
KZ_IBAN_RE = re.compile(r"\bKZ\d{20}\b")   # KZ + 20 цифр
BIC_RE     = re.compile(r"\b[A-Z]{6}[A-Z0-9]{2}([A-Z0-9]{3})?\b")

def norm_amount(x:str|None)->str|None:
    if not x: return None
    s = x.replace(" ", "").replace("\u00A0","").replace(",", ".")
    # оставим только цифры и одну точку
    m = re.findall(r"\d+\.?\d*", s)
    return m[-1] if m else None

def norm_currency(t:str)->str|None:
    if not t: return None
    t = t.upper()
    if "₸" in t or "KZT" in t: return "KZT"
