...
# Безопасный импорт исключений (чтобы имена всегда были определены)
try:
...
MAX_CTX = 20000  # ограничение контекста
...
SYSTEM_RU = (
    ...
    "date в формате YYYY-MM-DD. Учитывай русские синонимы: «итого», «к оплате», "
    "«сумма», «счет/счёт/№/№», «БИК/SWIFT», «IBAN/ИИК», «ИНН/ИИН/БИН». "
    "Если встречаешь «₸» или слово «тенге» → currency=KZT."
)
...
# Короткие русские примеры для few-shot
FEW_SHOTS_RU = [
    {
        "text": (
            "СЧЁТ № 123 от 10.09.2025\n"
            "Плательщик: ТОО «Ромашка»\nПолучатель: АО «Банк Демонстрации»\n"
            "IBAN (ИИК): KZ12345678901234567890  BIC: HSBKKZKX\n"
            "К оплате: 12 345,67 ₸"
        ),
        "out": {
            "fields": {
    ...
    {
        "text": (
            "Счет: INV-77  Дата: 3 декабря 2024 г.\n"
...
def _try_parse_ru_date(t: str) -> Optional[str]:
    # 3 декабря 2024 г. / 3 декабря 2024
    m = re.search(
        r"(?i)\b(0?[1-9]|[12]\d|3[01])\s+("
...
class LLMClient:
    ...
    def map_to_fields(
        ...
        # Поднимаем модель с русской системной инструкцией
        model_obj = genai.GenerativeModel(
            model or self.model_name,
            system_instruction=SYSTEM_RU,
        )

        # Собираем few-shot блоки
        shot_blocks = []
        ...
        user = (
            ...
            f"Подсказки (кандидаты): {hints_str}\n\n"
    ...
    # -------- офлайн-фолбэк (русский/казахский) --------
    def _fallback(self, doc_type: str, text: str, hints: Optional[Dict[str, Any]]) -> Dict[str, str]:
        t = text or ""
        fields: Dict[str, str] = {}

        # amount — больше русских синонимов
        m = re.search(
            r"(?i)(итог(?:овая)?\s*сумма|итого(?:\s*к\s*оплате)?|сумма\s*к\s*оплате|к\s*оплате|total|amount|sum)"
            r"\D{0,40}([0-9]+(?:[ .][0-9]{3})*(?:[.,][0-9]{2})?)",
            t,
        )
        if not m:
        ...
        if d:
            s = d.group(1)
            # нормализация DD.MM.YYYY → YYYY-MM-DD
